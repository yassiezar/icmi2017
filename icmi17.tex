...
From Figure~\ref{fig:tone-guesses} we can see that the number of samples for the higher, and therefore easier to differentiate, tone differences. We also see far fewer samples for the very small \SI{1}{\hertz} difference, which indicates that the subjects had trouble consistently reaching this difficulty and far more in correctly guessing the difference. Furthermore, the proportion of incorrect guesses in relation to correct guesses is higher for the smaller tone differences where it becomes more difficult to differentiate the two tones. 

These results are in line with one would expect, where a typical user would have difficulty differentiating two tones with a small frequency difference and we can see that the subjects can reliably differentiate two tones with a difference greater than \SI{128}{\hertz} [CHECK, skryf neer \% en maak wetenskaplik].
...
In Figure~\ref{fig:location-guesses}, with the relatively low number of red, incorrect guesses, we see that the subjects were far more successful in correctly guessing the location of the sound source. This is further supported by the large number of samples at the \SI{0.03125}{\m}, which was the minimum achievable level for this test, indicating that the subjects reached this level more frequently and consistently. 

Again, these results  in line with what we expected and is supported by literature which indicates that humans are very adept at localising the location of a sound source and this capability was apparent for HRTF-generated pan location. 

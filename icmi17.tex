...
To this end, we propose a mobile device-based navigation system that caters to the needs of the blind and visually impaired that is based on a Google Project Tango device. A Tango-enabled device comes pre-equipped with powerful image-processing, localisation and depth-perception capabilities and is built on top of a standard Android platform, giving us access to the entire set of input/output options that Android has to offer. The proposed system will use multiple feedback modes to guide a user toward a target destination while providing information on any oncoming obstacles.

The contributions to the current body of knowledge are two-fold: these are the first experimental results obtained that provide an indication on how well a tone with varying pitch can convey the elevation angle of a target and it is based on a hand-held mobile device that does all of the processing and uses a nondescript hand-manipulated camera instead of a head-mounted camera sensor. 
...
\subsection{Panning Dimension Results}

Figures~\ref{fig:pan-err-lo} to~\ref{fig:pan-err-hi} show the results for the errors in the pan dimensions from the target search experiment. These results are given as an angular error between the target's pan angle, i.e. the difference between the angle required to point to the target from the subject's current position, and the subject's current orientation angle. The figures are split up into the `low', `medium' and `high' pitch gain rates discussed earlier. 
...
The distributions in the pan dimension from Figures~\ref{fig:pan-err-lo} to~\ref{fig:pan-err-hi} are roughly centred around a zero-degree mean error, indicating no directional bias in the pan dimension. Furthermore, the histograms of the data look roughly normally distributed as well when compared to the reference red line, which is a normal distribution drawn using the mean and standard deviation parameters from the error data. 

Performing an ANOVA test on the pan error data between the pitch gradient settings reveals a $P$-level of 0.0005638. This is significantly lower than the 0.005 that is commonly used in literature [CITE?] and we can therefore reject the null-hypothsis that the three datasets have the same median and can conclude that the differences between them are statistically significant. This is an interesting result, since the adjustments made between the three datasets do not influence the behaviour of the panning instructions. Based on these plots, the `medium' pitch gradient setting produces the smallest standard deviation, while the average angular error between the datasets remains fairly similar. 

% Maak Fitts law grafiek wat lineere verhouding toon. 
% Time to target vir die verskillende data

\subsection{Tilt Dimension Results}
